require 'rails_helper'RSpec.describe 'Posts', type: :system, js: true do
  describe 'index page' do
    before(:example) do
      ...
      @comment5 = Comment.create(author: @user, post: @post1, text: 'I like this post')      visit user_posts_path(@user)
    end    it "renders user's profile picture" do
      find("img[src='#{@user.photo}']")
    end    it "renders the user's username" do
      expect(page).to have_content(@user.name)
    end    it "renders the user's post count" do
      expect(page).to have_content(@user.posts_counter)
    end    it "renders the user's posts" do
      expect(page).to have_content(@post4.title)
      expect(page).to have_content(@post2.title)
      expect(page).to have_content(@post3.title)
    end    it "renders some of the post's body" do
    ...
    end    it 'renders first comments on a post' do
      expect(page).to have_content(@comment1.text)
      expect(page).to have_content(@comment2.text)
    end    it 'renders comments count on a post' do
    ...
    end    it 'renders likes count on a post' do
    ...
    end    it 'renders pagination section if posts are more than 3' do
      expect(page).to have_content('Next →')
    end    it "redirects to a post's show page" do
  ...
  describe 'show page' do
    before(:example) do
      ...
      @comment3 = Comment.create(author: @user2, post: @post, text: 'I like this post comment three')      visit user_post_path(@user, @post)
    end    it "renders the post's title" do
      expect(page).to have_content(@post.title)
    end    it "renders the post's author" do
      expect(page).to have_content(@user.name)
    end    it 'renders comments count of the post' do
      expect(page).to have_content(@post.comments_counter)
    end    it 'renders likes count of the post' do
      expect(page).to have_content(@post.likes_counter)
    end    it "renders the post's body" do
      expect(page).to have_content(@post.text)
    end    it 'renders the username of each commenter' do
      expect(page).to have_content(@user2.name)
      expect(page).to have_content(@user.name)
    end    it 'renders all comments of the post' do
      expect(page).to have_content(@comment1.text)
      expect(page).to have_content(@comment2.text)
